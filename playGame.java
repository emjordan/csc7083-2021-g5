...
public class playGame {
	...
	public static boolean checkAllFlagsPlanted(ArrayList<Element> boardGameElements, String system, int currentSpace,
		...
		if (flagStatus = true) {
			String custodian = myPlayer.getPlayerName();
			for (int counter = 0; counter < boardGameElements.size(); counter++) {
				myElement = boardGameElements.get(counter);
				if (myElement.getSystemName() == system) {
	...
	/*
	 * public static void move(int counter) { Dice roll = new Dice(); int
	 * rollOutcome = roll.rollDice(); Player activePlayer = players.get(counter);
	 * System.out.println(activePlayer + " you rolled " + rollOutcome);
	 * 
	 * System.out.println();
	 * 
	 * System.out.println(activePlayer.getSpace()); int move =
	 * activePlayer.getSpace() + rollOutcome; System.out.println( activePlayer +
	 * " you are in space " + activePlayer.getSpace() +
	 * " and you will move to space " + move);
	 * 
	 * if (move > NUM_SPACES_ON_BOARD) { activePlayer.setSpace(move -
	 * NUM_SPACES_ON_BOARD); // this effectively resets the start count to 1 to
	 * start // the count from Cape Canaveral again for that player.
	 * 
	 * if (activePlayer.getSpace() != 1) { // in other words they are not on Cape
	 * Canaveral
	 ...
	 * System.out.println("You have arrived1" +
	 * " at Cape Canaveral!  Collect 30 units"); System.out.println();
	 * activePlayer.setNASAcredits(30); } } else {
